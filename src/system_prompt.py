...
def generate_system_prompt_with_norms(norms_content: str):
    """
    Generate system prompt with specific norms content.

    Args:
        norms_content: Pre-loaded norms content (from load_specific_norms)

    Returns:
        Complete system prompt with norms
    """
    template = load_system_prompt_template()
    return template.render(all_norms=norms_content)